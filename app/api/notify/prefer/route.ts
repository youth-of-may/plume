...
function shouldSendNow(sendTime: string): boolean {
    ...
    return now.startsWith(sendTime.substring(0, 5));
}
...
export async function GET(req: Request) {
    ...
    // Fetch all incomplete upcoming tasks
    ...
    // Build settings map by user_id
    const settingsMap = Object.fromEntries(
        (allSettings ?? []).map(s => [s.user_id, s])
    );

    // Build userMap from allUsers
    ...
    // Send task emails — respect opt-in, send time, days_before
    const taskResults = await sendInBatches(
        ...
        async ([userId, userTasks]) => {
            ...
            // Skip if opted out
            if (prefs && !prefs.tasks_enabled) return;

            // Skip if not their preferred send time
            const sendTime = prefs?.tasks_send_time ?? '08:00:00';
            if (!shouldSendNow(sendTime)) return;

            // Filter tasks within days_before window
            const daysBefore = prefs?.tasks_days_before ?? 1;
            const filteredTasks = userTasks.filter(t => {
                const daysUntil = Math.ceil(
                    (new Date(t.task_deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
                );
    ...
    // Send event emails — respect opt-in, send time, days_before
    const eventResults = await sendInBatches(
        ...
        async ([userId, userEvents]) => {
            ...
            // Skip if opted out
            if (prefs && !prefs.events_enabled) return;

            // Skip if not their preferred send time
            const sendTime = prefs?.events_send_time ?? '08:00:00';
            if (!shouldSendNow(sendTime)) return;

            // Filter events within days_before window
            const daysBefore = prefs?.events_days_before ?? 1;
            ...
            const emailEvents = filteredEvents.map(e => ({
    ...
    // Send journal reminder — respect opt-in and send time per user
    const journalResults = await sendInBatches(
        allUsers ?? [],
        async (user) => {
            if (!user.email) return;

            const prefs = settingsMap[user.id];

            // Skip if opted out
            if (prefs && !prefs.journal_enabled) return;

            // Skip if not their preferred send time
            const sendTime = prefs?.journal_send_time ?? '20:00:00';
            if (!shouldSendNow(sendTime)) return;

            const html = await render(
                JournalReminderEmail({
                    userName: user.user_metadata?.name ?? user.email ?? 'there'
                })
            );
            return resend.emails.send({
                from: 'Plume <plume@codedbymay.com>',
